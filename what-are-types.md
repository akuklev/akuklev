...
Let us begin with the most obvious issue of accidental information loss. Consider a language with an implicit conversion from `int32` to `float32`. While `int32` can store numbers between -2^32 and 2^32-1, `float32` is capable representing numbers between approx. -2^38 and 2^38, thus such a conversion never leads to an overflow or an underflow. Yet, it leads to a precision loss: many 32 bit integers, say 20000000 and 20000001 would be represented by the same `float32`. Such conversion does not render the program obviously wrong: many programs would still calculate sensible results. Yet, it leads to information loss that easily goes unnoticed while leading to wrong outcomes.

It gets even more tricky when parametrized data types like `List<T>` and `Map<X, Y>` come into play. A parameter `T` of a parametrized data type `SomeType<T>` is called covariant if there is a canonical lifting (usually called `map`) from functions `X -> Y` into functions `SomeType<X> -> SomeType<Y>` that respects function composition. The parameter `T` of `List<T>` is clearly covariant: there, the lifting `map` takes a function `f : X -> Y` and applies it to each element of a list `l : List<X>` producing a `List<Y>`. The type `Map<X, Y>` (of key-value dictionaries) can be seen as a generalization of the type `List<Y>`: it also contains a finite collection of items of fixed type `Y`, but in contrast to lists the items are indexed not by numbers, but by keys of the type `X`. The second parameter (`Y`) of `Map<X, Y>` is thus also covariant.
