I work at [HoTT and Dependent Types Group](https://research.jetbrains.org/groups/group-for-dependent-types-and-hott) at [JetBrains Research](https://research.jetbrains.org/). We study a particular kind of type systems and their applications in programming languages and pure mathematics. This article series is an attempt to explain our field to an interested reader with a decent exposure to programming. Those with a strong opinion on static typing are especially welcome.

§ Types: a Brief Introduction
-----------------------------

Data types such as `int`, `List<int>`, etc. are the types you might be familiar with. Besides programming languages, types are also present in formalized languages for mathematical theorems and their machine-checkable proofs. Semantically, type is a space of possible objects a variable can refer to. Syntactically, type is a (syntactic) attribute of a variable that determines available operations and specifies the behaviour of these operations. Each formalized language has its own type system, which guides its conceptual structure. Languages, where semantic and syntactic aspects of types are in harmony, are called strictly typed languages. At the moment (2020), none of the mainstream programming languages is strictly typed. Most [widespread](https://www.tiobe.com/tiobe-index/) [programming](https://pypl.github.io/PYPL.html) [languages](https://githut.info/) have ad hoc type systems (dynamically-typed languages being not exempt) tending to be incoherent in one way or another.
...
Programming language designers introduced types in the late 1950s for entirely unrelated reasons: variables had to have type declarations to tell the machine which processor register and/or how much memory storage space should be used for a given variable. Eventually, programming languages with complex type systems emerged. To rectify incoherences of those ad hoc type systems, computer scientists rediscovered mathematicians’ work on type theory in the 1970s. Since then, type theory has been developed by mathematicians and computer scientists hand in hand.

The practical goal of redesigning type systems of general-purpose programming languages is still far from being achieved. In fact, based on their experience with languages like C++, C# or Java, many programmers believe complex type systems to be a pointless pain in the neck. While some type-theoretically sound languages (e.g. the ML family) are there for almost half a century, type systems of most mainstream languages are a type theorist's nightmare. However, types are inevitable in programming languages, and carefully designing a type system in advance is the only way for typing not to be a nuisance.
...
2) There is a gap between statically-typed languages and dynamically-typed languages that has to be closed;
3) There are computational behaviours and programming techniques for which best typing practices are yet to be determined.

The first issue seems to be due to a communication gap between computer scientists and engineers, and huge systemic inertia. One cannot simply start from scratch:
not only backwards compatibility has to be retained, but there are many millions of software developers, who cannot learn unfamiliar programming languages and adopt new practices at once. Every serious shift is a tremendous effort to the industry of that size. We hope that availability of nice general-purpose languages where types are not a nuisance but an aid would help solving this issue, especially if those languages would be attractive for educators.

The second issue refers to an apparent dilemma: it is highly beneficial to use explicit type annotations (as in statically-typed languages) for public APIs, settled libraries, and critical sections where you need strong correctness guarantees, as well as for type-driven development, yet omitting them (as in dynamically-typed languages) is indispensable for a language with a complex type system to have a bearable learning curve and perform well at rapid prototyping. Yet can have both options in the same language by means of gradual typing and type inference: two mechanisms that allow omitting type annotations almost entirely in tractable cases. Gradual typing and type inference are readily available to some extent in some of mainstream languages including C# and Scala. The interplay between gradual typing and other features of complex type systems is, however, highly non-trivial.
...
Almost all existing type-theoretically sound languages are purely functional languages (such as Standard ML, Haskell, etc). That is not because good type systems are unavailable for other languages, but rather because purely functional languages require much simpler type systems. In purely functional languages, variables always refer to values, i.e. immutable pieces of data. Сonsequently, their type systems have to deal only with _data_ types which are well understood. On the other hand, types of varables referring to mutable objects and non-clonable resources (such as files, IO-streams, remote services or devices) are called _entity_ or _object_ types. Proper handling of object types is much more complex than that of data types. However, innate treatment of concurrency, mutable state and communication neccessarily requires using entity types besides data types.

Generic programming is an advanced programming technique allowing to lift algorithms and data structures from concrete examples to their general and abstract forms (definition due to Alexander Stepanov and Bjarne Stroustrup). In its advanced form, generic programming would allow to scrap boilerplate code entirely. However this promise is yet to be fulfilled in a real programming language. Basic forms of generic programming are available as “templates” in C++, “generics” in Java/C#, etc. One can observe, that parameters of types behave quite differently than usual values. The proper handling of generic programming calls for yet another class of types: the _purely abstract_ types, which are types of those “compile-time” parameters. Without innate support for purely abstract types, advanced generic programming (as available for example in D, Scala and Haskell) runs into awkward (performed in a very roundabout way) type-level programming.

Both _entity_ and _purely abstract_ types are crucial for developing good typing discipline for desirable computational behaviours and programming techniques and will be discussed at length below, as they constitute the central topic of our research regarding programming languages. While these kinds of types appeared in literature under numerous names many times, the pivotal improvement in their understanding was achieved by C. McBride in his 2016 note [“I got plenty o' nuttin'”](https://pureportal.strath.ac.uk/en/publications/i-got-plenty-o-nuttin).
...
```
map(0X̲ 0Y̲ : *, 0n̲ : Nat, l̲ : List<X, length = n>, f̲ : Function<X, T>) : List<Y>
```
