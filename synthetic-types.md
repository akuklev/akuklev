...
We believe that the best approach to this hefty topic is to work our way through with examples of increasing complexity. Since no mainstream programming language supports declarative data type definitions in sufficient generality, a pseudocode will be used in all examples. We will start with the definitions of finite types and describe how to recover primitive numeric data types (such as the type `int32` of 32 bit integers) declaratively. Then we will progressively define the types for unbounded natural, integer, and rational numbers. After that we will make a digression to define container types (such as pairs, lists, binary trees, etc.) and complex trees. Finally, we will outline the conceptual basis for declarative definition of real numbers.
