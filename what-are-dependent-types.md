...
1) side effect-free manifestly terminating functions (henceforce called “pure functions”) would be distinguised from possibly “effectful” functions on the type level,
2) if the compiler manages to check that it employs no side effect and terminates on every valid input, a (possibly effectful) function can be cast to a pure function.
